...
public class SortTests extends SortTester {
	...
	void testSorting(int[] original, boolean positions) {
		...
		if (positions)
			assertTrue(Sort.sort(temp, false) == null);
		else
			Sort.sort(temp);
		assertTrue(ordered(temp));
		assertTrue(sameArray(temp, original));
		if (positions) {
			int positionsArray[] = Sort.sort(sorted, true);
	...
	public void testSortIntArray() {
		...
		testSorting(new int[] { 5, 4, 3, 2, 1,2,3,4,5,7,7}, false);
	...
	public void testSortIntArrayBoolean() {
		...
		testSorting(new int[] { 5, 4, 3, 2, 1,2,3,4,5,7,7}, true);
